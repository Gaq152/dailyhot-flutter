...
class HotListParams {
  ...
  const HotListParams({required this.type, this.forceRefresh = false});
  ...
  @override
  int get hashCode => Object.hash(type, forceRefresh);
}

/// 热榜数据 Provider（使用 family 支持不同类型）
/// 返回 DataResult 包含数据、来源和错误信息
final hotListProvider =
    FutureProvider.family<DataResult<HotListResponse>, HotListParams>((
      ref,
      params,
    ) async {
